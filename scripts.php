...
<script type="text/javascript">
    jQuery(document).ready(function () {
      jQuery.ajax({
            ...
            type: "GET",
            contentType: "application/json; charset=utf-8",
            url: "jsons/etkinlikozet.json",
            dataType: "json",
            success: function (data) {

              jQuery("#etkinlik1baslik").text(data[0].baslik)
              jQuery("#etkinlik1ozet").text(data[0].ozet)
              jQuery("#etkinlik1resim").attr('src',(data[0].resim))
              jQuery("#etkinlik1url").attr('href',("etkinlikdetay.php?id="+data[0].url))

              jQuery("#etkinlik2baslik").text(data[1].baslik)
              jQuery("#etkinlik2ozet").text(data[1].ozet)
              jQuery("#etkinlik2resim").prop("src",data[1].resim)
              jQuery("#etkinlik2url").attr('href',("etkinlikdetay.php?id="+data[1].url))
              ...
              for (var i = 2; i < data.length; i++) {
                var div = '<div class="one_fourth tt-column">'+
                   ...
                   '<h4>' + data[i].baslik + '</h4>' +
                   '<p>' + data[i].ozet + '</p>' +
                   ' <a href=etkinlikdetay.php?id=' +data[i].url+ '>Devamı</a>' +
                '</div>'
                var newDiv = jQuery(div)
                jQuery("#haberListesi").append(newDiv)
              }
